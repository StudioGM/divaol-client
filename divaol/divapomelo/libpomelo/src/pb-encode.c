...
static int pb_encode_boolean(pb_ostream_t *stream, const void *value);
...
static int checkreturn pb_encode_proto(pb_ostream_t *stream, const json_t *gprotos, const json_t *protos,
	...
	case PB_bool:
		if (json_is_true(value))
			...
			return 0;
		if (!pb_encode_boolean(stream, &bool_val)) {
            return 0;
        }
		break;
    default:
...
/* Encode a bool value.
 * You need to pass a pointer to a 1-byte wide C variable. */
static int checkreturn pb_encode_boolean(pb_ostream_t *stream, const void *value) {
    return pb_write(stream, (const uint8_t *) value, 1);
}
