...
namespace diva
{
	...
	void DivaPlayerList::ModifyPlayer(int index, const DivaPlayerInfo& inf)
	{
		...
		p->setInfo(inf.name, iconList[inf.iconIndex], gcn::Rectangle(0,0,0,0), L"res/UI2.png");
	...
	void DivaPlayerList::init()
	{
		...
		for (int i = 0; i < info.players.size(); i++)
		{
			...
			p->setInfo(info.players[i].name, iconList[info.players[i].iconIndex], gcn::Rectangle(0,0,0,0), L"res/UI2.png");
