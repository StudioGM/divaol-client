...
namespace diva
{
	...
	UIScreenRoom::UIScreenRoom()
	{
		...
		uiImage1 = gcn::Image::load("UI1.png");
		uiImage2 = gcn::Image::load("UI2.png");
		...
		backPanel->load(L"UI1.png", gcn::Rectangle(0, 890, 1859, 997));
		...
		songListBox->loadImage(L"UI1.png", gcn::Rectangle(613, 0, 609, 873));
		songListBox->loadButtonImage(L"UI1.png",
		...
		leftArtistButton->setLook(L"UI1.png", 
		...
		leftSongButton->setLook(L"UI1.png",
		...
		thumbFrameContainter->load(L"UI2.png", gcn::Rectangle(0,0,412,251));
		...
		songInfoContainer->load(L"UI2.png", gcn::Rectangle(412,414,553,295));
		...
		modeLeftButton->setLook(L"UI1.png", gcn::Rectangle(111, 326, 64, 51),
			gcn::Rectangle(111, 204, 64, 51),
			gcn::Rectangle(111, 262, 64, 51),
			gcn::Rectangle(111, 326, 64, 51),
			-14, -10);
		...
		modeRightButton->setLook(L"UI1.png", gcn::Rectangle(235, 326, 64, 51),
			gcn::Rectangle(235, 204, 64, 51),
			gcn::Rectangle(235, 262, 64, 51),
			gcn::Rectangle(235, 326, 64, 51),
			-14, -10);
		...
		const std::vector<Network::Map> & orig = MapMgr::Instance()->GetMaps();
		gcn::Image* image = gcn::Image::load("UI1.png");
		...
		for (int i=0; i<orig.size(); i++)
			for (std::vector<std::wstring>::const_iterator j = orig[i].artists.cbegin(); j != orig[i].artists.cend(); j++)
			{
	...
	void UIScreenRoom::Render()
	{
		//sprite->render(0, 0);
	}

	void UIScreenRoom::updateNet(float delta)
	{
		// SMDivaNetCode
	...
	void UIScreenRoom::Update(float delta)
	{
		// NetUpdate
		//updateNet(delta);
	...
	void UIScreenRoom::SongListItemClicked(int index)
	{
		if (state == SONGLIST_ART)
		{
			std::wstring des = ((SongListItem*)songListBox->getItems()[index])->getMapInfo().artists[0];
			songListBox->clearItems();
		
			songListBox->pushItem(songListOrigItems[0]);
			for (int i=0; i<songListOrigItems.size(); i++)
			{
				const std::vector<std::wstring>& p = ((SongListItem*)songListOrigItems[i])->getMapInfo().artists;
	...
	void UIScreenRoom::AdjustStartButton()
	{
		// if no song
	...
	void UIScreenRoom::SongListHighlightItemChanged(int index)
	{
		if (index == -1)
		{
	...
	void UIScreenRoom::GameStartPost(int mapId, int dif, int gameMode)
	{
		//SMDivaNetCode
		//////////////////////////////////////////////////////////////////////////
	...
	void UIScreenRoom::GameStart(int mapId, int dif, int gameMode, int myPos)
	{
	}

	void UIScreenRoom::GameOver()
	{
		//netCore.send(DIVA_NET_CTS_LOBBY_GAME_OVER);
	}

	void UIScreenRoom::ReadyPost(int index, bool p)
	{
	...
	{
		UIScreenRoom::Instance()->songListBox->setFirstIndex(v);
	}
	...
	void ArtistButton_MouseListener::mouseClicked(MouseEvent& mouseEvent)
	{
		...
		ui->state = UIScreenRoom::SONGLIST_ART;
	}

	void SongButton_MouseListener::mouseClicked(MouseEvent& mouseEvent)
	{
		UIScreenRoom::Instance()->state = UIScreenRoom::SONGLIST_ORIG;
		UIScreenRoom::Instance()->songListBox->setDifButtonDisplayed(true);
		UIScreenRoom::Instance()->songListBox->setItems(UIScreenRoom::Instance()->songListOrigItems);
	}
	...
	void GameModeSelectLeft_MouseListener::mouseClicked(MouseEvent& mouseEvent)
	{
		...
		UIScreenRoom::Instance()->AdjustStartButton();
	}
