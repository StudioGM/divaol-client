...
namespace lua_tinker
{
	...
	struct table_obj
	{
		...
		{ 
			lua_pushstring(m_L, name);
			lua_pushlightuserdata(m_L, (void*)func);
		...
		RVal call(const char* name, T1 arg)
		{
			...
			{
				print_error(m_L, "lua_tinker::call() attempt to table function `%s' (not a function)", name);
			}
		...
		RVal call(const char* name, T1 arg1, T2 arg2)
		{
			...
			if(lua_isfunction(m_L,-1))
			{
				push(m_L, arg1);
				push(m_L, arg2);
				if(lua_pcall(m_L, 2, 1, errfunc) != 0)
				{
					lua_pop(m_L, 1);
				}
			}
		...
		RVal call(const char* name, T1 arg1, T2 arg2, T3 arg3)
		{
			...
			if(lua_isfunction(m_L,-1))
			{
				...
				push(m_L, arg2);
				push(m_L, arg3);
				if(lua_pcall(m_L, 3, 1, errfunc) != 0)
				{
					lua_pop(m_L, 1);
				}
			}
	...
	struct table
	{
		...
		RVal call(const char* name, T1 arg)
		{
			return m_obj->call<RVal,T1>(name,arg);
		}

		template<typename RVal, typename T1, typename T2>
		RVal call(const char* name, T1 arg1, T2 arg2)
		{
			return m_obj->call<RVal,T1,T2>(name,arg1,arg2);
		}

		template<typename RVal, typename T1, typename T2, typename T3>
		RVal call(const char* name, T1 arg1, T2 arg2, T3 arg3)
		{
			return m_obj->call<RVal,T1,T2,T3>(name,arg1,arg2,arg3);
		}
