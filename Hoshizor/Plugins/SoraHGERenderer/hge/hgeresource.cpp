...
char* CALL HGE_Impl::Resource_EnumFiles(const char *wildcard)
{
	if(wildcard)
	{
		...
		hSearch=FindFirstFileW(char2wchar(Resource_MakePath(wildcard), -1), &SearchData);
		if(hSearch==INVALID_HANDLE_VALUE) { hSearch=0; return 0; }

		if(!(SearchData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) return wchar2char(SearchData.cFileName);
	...
	else
	{
		...
		for(;;)
		{
			if(!FindNextFileW(hSearch, &SearchData))	{ FindClose(hSearch); hSearch=0; return 0; }
			if(!(SearchData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) return wchar2char(SearchData.cFileName);
		}
	}
}
	...
	if(wildcard)
	{
		...
		if((SearchData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
		   wcscmp(SearchData.cFileName,L".") && wcscmp(SearchData.cFileName,L".."))
				return wchar2char(SearchData.cFileName);
