...
#ifndef Sora_SoraPerlinNoise_h
#define Sora_SoraPerlinNoise_h
...
namespace sora {
    ...
    inline float SoraPerlinNoise::fade(float t) {
        return t*t*t*(t*(t*6.0f-15.0f)+10.f);
    }
    
    inline float SoraPerlinNoise::lerp(float t, float a, float b) {
        return a + t * (b-a);
    }
    
    inline float SoraPerlinNoise::grad(int hash, float x, float y, float z) {
        int h = hash & 15;
        float u = h < 8 ? x : y;
